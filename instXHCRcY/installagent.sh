...
usage()
{
 ...
 if [ "$PRODUCT_ID" != "DCS" ]; then
   ...
   printf "\n\t  --update-kmod              Check for and update just the kmod package from the repository" 
...
installGPGKeys () 
{
  ...
  [ "$CONFIGURE_REPO" != true ] && [ $ISUPGRADE = true ] && log_msg "Skip key import on upgrade" && return 0
  [ ! -f $pkg_cert ] && log_msg "Warning: $pkg_cert missing" && return 0
  log_msg "Running key import for Symantec Repo"
  case $PKG_MGR in
    yum)
...
reportTelemetry()
{
  ...
  if ( [ "$rc_mktemp" = 0 ] && [ ! -z "$SDCSS_TMP_LOGDIR" ] && [ "$result" = 2 ] ); then
...
installAgent()
{
  ...
  for pkg in ${PACKAGES[@]}; do 
    ...
    if ( [ "$PRODUCT_ID" != "CWP" ] && [ "$PKG_MGR" != "apt" ] ); then
      dcspkgname="$pkg"
      if ( [ $pkg != sdcss-kmod ] && [ $pkg != $CAF_PACKAGE ] ); then
          dcspkgname="$pkg-[6-9]"
      fi
       log_msg "installAgent() dcspkgname: $dcspkgname"
       localfile="`ls -1tr "$PWD"/$dcspkgname*.$PLAT_PKG 2>/dev/null |sort -V |tail -1`"
    else
       localfile="`ls -1tr "$PWD"/$pkg*.$PLAT_PKG 2>/dev/null |sort -V |tail -1`"
    fi
    ...
    if [ "$localfile" ]; then
      [[ $localfile =~ $PKG_MASK ]] || error 1 "$localfile cannot be installed on $OS."
